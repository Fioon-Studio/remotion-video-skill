...
`highlightAnchors` 和 `beats` 的起止帧只来自 ASR 强制对齐。`rectangles` 是可测的排版登记，必须覆盖真实标题、说明、卡组、截图焦点和字幕预留区，用来在渲染前发现重叠，而不是靠事后肉眼找问题。
...
6. 导出音频若从原文件裁掉了 `k` 帧，规格必须声明 `audioTrimFrames: k`。页面、字幕和点亮统一以 `narrationFrame = outputFrame + k` 对照原始 ASR 帧；不能让音频和视觉各自再补一段延迟。
7. 同一列的相邻矩形要声明最小垂直间距并在渲染前检查。文字真实高度必须纳入矩形，不能只登记含它们的外层容器。
