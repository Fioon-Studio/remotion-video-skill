...
3. 字幕、画面、人物动作、截图框选、重点点亮和音效都从同一份真实时间线读取。每个变化以对应口播词或短句的起始帧和结束帧为锚点，不按场景平均分配。对“当前正在讲的项”必须使用 `startFrame <= frame < endFrame`：开始即亮、结束即回到暗态，不能只判断开始帧让最后一项持续亮着。60fps 时，将强制对齐的毫秒时间换算至最近一帧，不另加主观延迟。
