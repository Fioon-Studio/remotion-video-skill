...
3. 字幕、画面、人物动作、截图框选、重点点亮和音效都从同一份真实时间线读取。每个变化以对应口播词或短句的起始帧和结束帧为锚点，不按场景平均分配。对“当前正在讲的项”必须使用 `startFrame <= frame < endFrame`：开始即亮、结束即回到暗态，不能只判断开始帧让最后一项持续亮着。60fps 时，将强制对齐的毫秒时间换算至最近一帧，不另加主观延迟。完整导出前，在至少三个含人声的时间窗对导出音轨和最终 narration 做波形互相关；若发现一致的全局偏移，只按最接近的整帧统一裁切或预移音频，并复测到残差不超过半帧，不能将它误改成个别词的延迟。
...
15. 规格必须登记真实标题、说明、卡组、截图焦点和字幕预留区的矩形，不能只登记一个笼统容器；同列相邻元素需通过最小垂直间距检查。若导出音频裁掉开头 `k` 帧，规格记录 `audioTrimFrames: k`，所有视觉高亮以 `outputFrame + k` 对照原始 ASR 帧，不能把音频裁切变成词级错位。
16. 英文缩写或产品词若被 ASR 合成一个词块，禁止按字符均分其时长。截取该组词的小音频片段，使用带空格的完整词表单独强制对齐；只有每个词都有独立起止时间，才能覆盖通用对齐结果并进入渲染。
